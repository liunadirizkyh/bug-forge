...
export function parseMarkdownReport(markdown: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("**Vulnerability Type:**") || line.startsWith("**Tipe Kerentanan:**")) {
      vulnType = line
        .replace(/\*\*(Vulnerability Type|Tipe Kerentanan):\*\*/i, "")
        .replace(/\*\*/g, "")
        .replace(/`/g, "")
        .trim();
      continue;
    }
    if (line.startsWith("**Severity:**") || line.startsWith("**Tingkat Keparahan:**")) {
      severity = line
        .replace(/\*\*(Severity|Tingkat Keparahan):\*\*/i, "")
        .replace(/\*\*/g, "")
        .replace(/`/g, "")
        .trim();
      continue;
...
function drawSeverityBadge(doc: jsPDF, severity: string, x: number, y: number) {
  ...
  if (sevLower.includes("critical") || sevLower.includes("kritis")) {
    badgeColor = [220, 38, 38]; // Red
  } else if (sevLower.includes("high") || sevLower.includes("tinggi")) {
    badgeColor = [234, 88, 12]; // Orange
  } else if (sevLower.includes("medium") || sevLower.includes("sedang")) {
    badgeColor = [217, 119, 6]; // Amber
  } else if (sevLower.includes("low") || sevLower.includes("rendah")) {
...
export function exportReportToPDF(markdownText: string) {
  ...
  const labelTarget = "TARGET HOST / URL";
  const labelSeverity = "SEVERITY LEVEL";
  const labelVulnType = "VULNERABILITY TYPE";
  const labelPlatform = "PLATFORM";
  const labelDate = "Report Date";
  const labelFooter = "Confidential Security Report  |  Generated by BugForge";
  ...
  doc.text(`${labelDate}: ${dateStr}`, 190, currentY + 4, { align: "right" });
  ...
  doc.text(labelTarget, col1X, itemY);
  doc.text(labelSeverity, col2X, itemY);
  ...
  doc.setFontSize(8);
  doc.setTextColor(100, 116, 139);
  doc.text(labelVulnType, col1X, itemY);
  doc.text(labelPlatform, col2X, itemY);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    // Left-aligned footer
    doc.text(labelFooter, 20, 285);
    
    // Right-aligned page number
    doc.text(`${labelPage} ${i} ${labelOf} ${totalPages}`, 190, 285, { align: "right" });
  }
